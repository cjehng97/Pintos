...
static const int MAX_ARG_SIZE = 50;
...
static void argument_number_and_length
  (char ** argv, int * argc, int * len_of_argv);
...
/*
 * Returns the size of string s. Probably same as strlen
 */
static int
string_size(char* s) {
  int i;
...
/*
 * Flattens argc arguments from argv into the string flattened
 * ex: ["x", "xy", "abc"] -> "x xy abc\0"
 */
static void
cmd_line_flatten(char * flattened, int argc, char ** argv) {
  int i, j;
  int sp = 0;
  for (i=1; i<=argc; i++) {
    for (j=0; argv[i][j]!='\0'; j++) {
      flattened[sp++] = argv[i][j];
    }
    flattened[sp++] = ' ';
  }
  flattened[sp] = '\0';
...
/*
 * PRE: ...
 * POST: returns the number of arguments in argv inside argc
 * POST: returns the total number of characters from all the strings
 * inside argv (including spaces and terminating NULL)
 */
static void
argument_number_and_length(char ** argv, int * argc, int * len_of_argv) {
  int i;
  for (i=1; argv[i]!=NULL; i++) {
	(*argc) ++;
    (*len_of_argv) += string_size(argv[i]);
  }
}
...
run_task (char **argv)
{
  ...
  int len_of_argv;
  int argc = 0;
  argument_number_and_length(argv, &argc, &len_of_argv);
  char flattened_cmd_line[len_of_argv];
  cmd_line_flatten(flattened_cmd_line, argc, argv);

  printf ("Executing '%s':\n", task);
#ifdef USERPROG
  process_wait (process_execute (flattened_cmd_line));
#else
  run_test (task);
#endif
